...
/**
 * @author ${Umesh} on 12-06-2018.
 */

public class GiveAwayTimerFragment extends Fragment {
    ...
    private DatabaseReference dbInComingRef;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        String requestText = preferences.readString(Constants.GIVE_AWAY_TIMER_TITLE);
    ...
    private void changeStringColor(String requestText) {
        if(requestText!=null){
            ...
            String red = "Your request for  giving ";
    ...
    private void startTimerTask() {
        ...
        long mil = preferences.readLong(Constants.GIVE_AWAY_TIMER_TIME);
        ...
        if(timeRemains>=0){
            timer  = new CountDownTimer(timeRemains, 1000) {
                ...
                public void onFinish() {
                    ...
                    preferences.writeBoolean(Constants.GIVE_AWAY_TIMER_VISIBLE,false);
                    preferences.writeLong(Constants.GIVE_AWAY_TIMER_TIME,0);
                    showRequestExpireAlert();
                }
            }.start();
        }else {
            upDateTimeTextView.setText("done!");
    ...
    private void showRequestExpireAlert() {
        if(getActivity()!=null){
            final Dialog dialog = new Dialog(getActivity());
    ...
    private void cancelRequest() {
        isCancelClick = true;
        String str = preferences.readString(Constants.Give_AWAY_REQUEST_KEY);
    ...
    public void saveExtendTimeToPref(int timeDuration, String unit){
        if(unit.equalsIgnoreCase("Minutes")){
            ...
            preferences.writeLong(Constants.GIVE_AWAY_TIMER_TIME,System.currentTimeMillis()+timeInMaliSecond);
    ...
    private void updateTimeInRequest(long timeInMaliSecond) {
        ...
        DatabaseReference databaseOutstanding = database.getReference("Outstanding").child(preferences.readString(Constants.USER_ID)).child("1");
        HashMap hashMap = new HashMap();
        hashMap.put("timeout",timeInMaliSecond);
        hashMap.put("timestamp",System.currentTimeMillis());
        databaseOutstanding.updateChildren(hashMap);
        DatabaseReference databaseReference =  FirebaseDatabase.getInstance().getReference("Incoming").child(preferences.readString(Constants.Give_AWAY_REQUEST_KEY));
    ...
    public void requestEnteredInIncoming() {
        ...
        String requestKey = preferences.readString(Constants.Give_AWAY_REQUEST_KEY);
        ...
        incomingEventListener = new ChildEventListener(){
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                btnCancel.setVisibility(View.VISIBLE);
                btnExtends.setVisibility(View.VISIBLE);
                preferences.writeBoolean(Constants.GIVE_REQUEST_ACTIVE,true);
            }
        ...
        if(requestKey!=null){
            dbInComingRef = FirebaseDatabase
                    .getInstance()
                    .getReference("Incoming")
                    .child(requestKey);
            dbInComingRef.addChildEventListener(incomingEventListener);
        }
    ...
    public void onDestroy() {
        super.onDestroy();
        if(dbInComingRef!=null){
            dbInComingRef.removeEventListener(incomingEventListener);
        }
    }
